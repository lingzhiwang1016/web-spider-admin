export const patten = {
    ...
    // 请最多保留两位小数点
    floatprice: /^[1-9](\d){0,9}?(\.(\d){1,2})?$|0$/,
    // 请输入正确的数字格式,末尾是点(.)的浮点数
    floatpriceright: /^[1-9](\d){0,9}?(\.(\d+))?$|0$/,
    // 请最多保留两位小数点,超过两个小数的浮点数
    floatpricetwo: /^[1-9](\d){0,9}?(\.(\d+)?)?$|0$/,
    // 请输入数字
    floatpriceall: /^[1-9](\d+)?$|0$/
};
...
const funcs = {
    ...
    funcDelay(delay = 300) {
        ...
        //     if (!preTime || preTime < nowTime - delay) {
