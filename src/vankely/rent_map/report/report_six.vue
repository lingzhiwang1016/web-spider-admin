<template lang="html">
    <div class="report_option6 item"></div>
</template>
...
export default {
    ...
    methods: {
        ...
        async getChartData(res) {
            if (res) {
                ...
                if (res.trafficHubPoiList.length < 1) return;
                const arr = res.trafficHubPoiList.filter(item => {
                    return item.coverageRadius === Number(coverageRadius).toFixed(2) + "" && item.houseType === houseType;
                });
                //this.$logger.log("按交通枢纽划分的租房数量和平均房价关系图", arr, coverageRadius, houseType);
                if (arr.length < 1) {
                ...
                arr.forEach((item) => {
                    ...
                    seriesData[1].push(item.roomCharge);
    ...
    mounted () {
        this.charts = echarts.init(document.getElementsByClassName("report_option6")[0]);
    }
};
</script>
