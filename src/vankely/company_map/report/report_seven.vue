<template lang="html">
    <div class="report_option7 item"></div>
</template>
...
export default {
    ...
    methods: {
        ...
        async getChartData(res) {
            if (res) {
                ...
                const seriesData = [[], []];
                if (res.industryRecruitmentStatisticsList.length < 1) return;
                res.industryRecruitmentStatisticsList.forEach((item) => {
                    seriesData[0].push(item.enterpriseNum);
                    seriesData[1].push(item.salary);
                    xAxisData.push(item.industry);
                });

                // 获取所有数据中的最大值最小值
                const minMax0 = getMinMax(res.industryRecruitmentStatisticsList, "enterpriseNum");
                const minMax1 = getMinMax(res.industryRecruitmentStatisticsList, "salary");
                // 获取y轴的最大值最小值
        ...
        async initState(xAxisData, yAxisData, seriesData) {
            ...
            const xAxis = {
                type: "category",
                data: xAxisData,
                boundaryGap: commonStyle.commonX.boundaryGap,
                axisTick: commonStyle.commonX.axisTick,
                axisLine: commonStyle.commonX.axisLine,
                axisLabel: {
                    ...commonStyle.commonX.axisLabel,
                    interval: 0,
                    rotate: 40,
                    fontSize: 10
                }
    ...
    mounted () {
        this.charts = echarts.init(document.getElementsByClassName("report_option7")[0]);
    }
};
</script>
