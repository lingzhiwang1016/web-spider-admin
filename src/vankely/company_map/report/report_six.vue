    ...
    methods: {
        ...
        async getChartData(res) {
            if (res) {
                ...
                if (res.trafficHubPois.length < 1) return;
                const arr = res.trafficHubPois.filter(item => {
                ...
                // 获取所有数据中的最大值最小值
                const minMax0 = getMinMax(arr, "enterpriseNum");
                const minMax1 = getMinMax(arr, "salary");
                // 获取y轴的最大值最小值
