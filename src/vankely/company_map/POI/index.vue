...
export default {
    ...
    methods: {
        ...
        handerMarkerLabel(data) {
            ...
            data.forEach((item, index) => {
                ...
                if (!this.isCounty) {
                    this.makeCircle(point, item.salary);
                }
        ...
        makeCircle(point, data) {
            ...
            if (Number(data) <= 6000) {
                cicrleColor = "#00FF7F";
            } else if (Number(data) > 6000 && Number(data) <= 7500) {
                cicrleColor = "#FFC125";
            } else {
                cicrleColor = "#FF0000";
            }
        ...
        getParams() {
            this.$logger.log("month", this.$store);
            if (this.isCounty) {
                return {
                    city: this.city,
        ...
        // 单个文本编写
        getText(item, title) {
            const salary = !item.salary ? "暂无数据" : this.$utils.formatFloat2(item.salary);
            const recruitNum = !item.recruitNum ? "暂无数据" : item.recruitNum;
            const txt = `<p class="circleTitle">${title}</p>
                          企业数: ${item.enterpriseNum}</br>
                          平均薪资: ${salary}</br>
                          招聘人数: ${recruitNum}
                          `;
            return txt;
    ...
    created() {
        this.mainKey = "enterpriseNum";
        // 按区县统计的url
        this.districtLabelUrl = this.$api.recruitment_poi_county_list;
    },
    ...
    async mounted() {
        ...
        this.initState();
        // 按商圈、交通枢纽
        this.url = this.$api.recruitment_poi;
        this.getCountys();
    }
};
</script>
