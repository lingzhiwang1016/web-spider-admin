    ...
    methods: {
        ...
        async getChartData(res) {
            if (res) {
                ...
                const seriesData = [[], []];
                if (res.hotelTypeStatisticList.length < 1) return;
                res.hotelTypeStatisticList.forEach((item) => {
                    seriesData[0].push(item.hotelNum);
                    seriesData[1].push(item.lowestPrice);
                    xAxisData.push(item.hotelType);
                });

                // 获取所有数据中的最大值最小值
                const minMax0 = getMinMax(res.hotelTypeStatisticList, "hotelNum");
                const minMax1 = getMinMax(res.hotelTypeStatisticList, "lowestPrice");
                // 获取y轴的最大值最小值
