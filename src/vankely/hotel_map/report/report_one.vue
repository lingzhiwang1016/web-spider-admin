    ...
    methods: {
        ...
        async getChartData(res) {
            if (res) {
                const legendData = [];
                const seriesData = [];
                if (res.hotelTypeStatisticList.length) {
                    res.hotelTypeStatisticList.forEach((ele) => {
                        if (Number(ele.hotelNum) > 0) {
                            seriesData.push({ value: ele.hotelNum, name: ele.hotelType });
                            legendData.push(ele.hotelType);
                        }
                    });
                }
        ...
        async initState(legendData, seriesData) {
            const tooltip = {
                trigger: "item",
                formatter: this.labelShow ? "{b}" : "占比:{d}% <br/>酒店类型:{b} <br/>酒店数量:{c}家"
            };
            const legend = {
                data: legendData,
