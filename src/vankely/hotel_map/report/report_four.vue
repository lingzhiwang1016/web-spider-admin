    ...
    methods: {
        ...
        async getChartData(res) {
            if (res) {
                ...
                const seriesData = [[], []];
                if (res.hotelCountyStatisticList.length < 1) return;
                res.hotelCountyStatisticList.forEach((item) => {
                ...
                // 获取所有数据中的最大值最小值
                const minMax0 = getMinMax(res.hotelCountyStatisticList, "hotelNum");
                const minMax1 = getMinMax(res.hotelCountyStatisticList, "lowestPrice");
                // 获取y轴的最大值最小值
